...
import com.kaazing.gateway.amqp.client.AmqpClient.ReadyState;
import com.kaazing.gateway.amqp.client.ChannelEvent.Kind;
import com.kaazing.gateway.amqp.client.ChannelEvent.MethodName;
...
import com.kaazing.gateway.amqp.client.impl.ChannelStateMachineListener;
...
public final class AmqpChannel {
     ...
     * @return AmqpChannel
     * 
     * @Deprecated -- Use the other variant of publishBasic() method that accepts AmqpProperties as a parameter.
     */
    public AmqpChannel publishBasic(ByteBuffer body, HashMap<String, Object> headers, String exchange, String routingKey, boolean mandatory, boolean immediate) {
     ...
     * @return AmqpChannel
     */
    public AmqpChannel ackBasic(int deliveryTag, boolean multiple) {
     ...
     * @return AmqpChannel
     */
    public AmqpChannel rejectBasic(int deliveryTag, boolean requeue) {
    ...
    private void sendFrame(Object client, AmqpBuffer buffer) {
        try {
            if (this.client.getReadyState() == ReadyState.OPEN) {
                this.client.websocket.send(buffer);
            }
        }
        catch (Exception e) {
    ...
    void messageBodyHandler(Object context, String input, Object frame, String stateName) {
        AmqpFrame f = (AmqpFrame)frame;
        AmqpChannel channel = client.channels.get(f.getChannelId());
        ChannelEvent e = new ChannelEvent(channel, ChannelEvent.getKind(f), f.getArgs(), headerFrame.getContentProperties(), f.getBody());
