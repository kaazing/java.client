...
import com.kaazing.net.ws.amqp.ConnectionEvent.Kind;
import com.kaazing.net.ws.amqp.impl.Action;
import com.kaazing.net.ws.amqp.impl.AmqpBuffer;
import com.kaazing.net.ws.amqp.impl.AmqpBuffer.Arg;
import com.kaazing.net.ws.amqp.impl.AmqpBuffer.BasicProperties;
import com.kaazing.net.ws.amqp.impl.AmqpFrame;
import com.kaazing.net.ws.amqp.impl.AmqpMethod;
import com.kaazing.net.ws.amqp.impl.AsyncClient;
import com.kaazing.net.ws.amqp.impl.ClientChannelBehaviours;
import com.kaazing.net.ws.amqp.impl.ClientStateMachineListener;
import com.kaazing.net.ws.amqp.impl.Continuation;
import com.kaazing.net.ws.amqp.impl.ErrorHandler;
import com.kaazing.net.ws.amqp.impl.EventTargetSupport;
import com.kaazing.net.ws.amqp.impl.MethodLookup;
import com.kaazing.net.ws.amqp.impl.Rules;
...
public final class AmqpClient {
    ...
    /**
     * Returns the instance of {@link AmqpClientFactory} that was used to
     * to create this instance.
     * 
     * @return instance of {@link AmqpClientFactory}
     */
    ...
    void handshakeStartHandler(Object context, String input, Object args, String stateName) {
        try {
            WebSocketFactory wsFactory = amqpClientFactory.getWebSocketFactory();
            websocket = wsFactory.createWebSocket(new URI(args.toString()));
            websocket.connect();
            messageReader = websocket.getMessageReader();
            startReceivingMessage();
            
            // connect() is a blocking call. When the call returns, it means that the WebSocket connection 
        ...
        Thread messageReceivingThread = new Thread() {
            public void run() {
                ...
                try {
                    ...
                    while ((type = messageReader.next()) != WebSocketMessageType.EOS) {
                        switch (type) {
                            case BINARY:
                                ...
                                WrappedByteBuffer wrappedBuffer = WrappedByteBuffer.wrap(data);
                                socketMessageHandler(wrappedBuffer);
                                break;
                            case TEXT:
                    ...
                    // listener.onClose should only be called from here if the connection get closed forcibly due 
                    // to network failure or the Gateway got shut down.
                    ...
                    //if (GenericLogger.isLoggable(Level.FINE)) {
                    //    GenericLogger.fine(ex.getMessage() + "\n" + ex.getStackTrace());
                    //}
    ...
    void send(WrappedByteBuffer buf) throws IOException {
        websocket.getMessageWriter().writeBinary(buf.getNioByteBuffer());
    }
    ...
    void tuneConnectionHandler(Object context, String input, Object args, String stateName) {
        ...
        // By sending heartbeat = 0 via tune-ok, we are letting the broker know
        // that the client does not want heartbeat.
        int heartbeat = 0; // (Integer)frame1.getArgs()[2].value;
    ...
    private void socketOpenHandler() {
        ...
        WrappedByteBuffer headerbuf = WrappedByteBuffer.allocate(512);
    ...
    private void socketMessageHandler(WrappedByteBuffer wrappedBuffer) {
        inBuffer.putBuffer(wrappedBuffer);
